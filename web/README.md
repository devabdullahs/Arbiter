...
- Discord-linked users can select a server from their Discord account when
  creating an org. A manual server ID field remains as a fallback, and the UI
  provides a preselected Add Bot link when the selected server is not configured.
